...
export interface UploadMedicationPhotoParams extends UploadBillPhotoParams {
  timezone?: string;
}
...
export async function uploadMedicationPhoto({
  ...
  contentType,
  timezone,
}: UploadMedicationPhotoParams): Promise<UploadPhotoResponse> {
  const resolvedName = fileName ?? uri.split('/').pop() ?? 'photo.jpg';
  const type =
    contentType ??
    (() => {
  ...
  const query = new URLSearchParams({ intent: 'medication' });
  if (timezone) {
    query.set('timezone', timezone);
  }

  const url = `${API_ENDPOINTS.uploadPhoto}?${query.toString()}`;

  const response = await apiClient.post<UploadPhotoResponse>(url, formData, {
    headers: { 'Content-Type': 'multipart/form-data' },
  });
