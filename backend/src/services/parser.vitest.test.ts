...
describe('parser service', () => {
  ...
  it('overrides to bill when monetary signals present', () => {
  ...
  it('does not misclassify parking fine OCR as bill', () => {
  ...
  it('still classifies concise bills when structural signals present', () => {
    const conciseBill = `
      STATEMENT
      Amount Due $75.00
      Pay by 11/05/2030
  ...
  it('extracts appointment data correctly', () => {
    ...
    assert.ok(
      appointment.location && appointment.location.includes('Valley Medical'),
      'Should extract location'
    );
    assert.ok(
      appointment.prepNote && appointment.prepNote.includes('15 minutes early'),
      'Should extract prep note'
    );
  });

  // Test bill extraction
  it('extracts bill data correctly', () => {
    const text = `
      Medical Bill
    ...
    assert.strictEqual(bill.amount, 145.5, 'Should extract amount in dollars');
    assert.ok(
  ...
  it('extracts realistic bill fields from OCR text', () => {
    const text = `
  ...
  it('parseSource marks overdue bills', () => {
    const source = {
      id: 99,
      recipientId: 1,
  ...
  it('parseSource creates correct structure for appointment', () => {
    const source = {
      id: 1,
      recipientId: 1,
      kind: 'email' as const,
  ...
  it('parseSource creates correct structure for bill', () => {
    const source = {
      id: 2,
      recipientId: 1,
      kind: 'email' as const,
  ...
  it('prefers full text when provided', () => {
    const source = {
      id: 3,
      recipientId: 1,
      kind: 'upload' as const,
