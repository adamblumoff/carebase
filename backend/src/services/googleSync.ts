...
const MANAGED_CALENDAR_ACL_REFRESH_INTERVAL_MS = 12 * 60 * 60 * 1000; // 12 hours
...
export async function ensureManagedCalendarForUser(
  ...
  if (!calendar) {
    ...
    const match = calendars.find(
      (entry) => isManagedCalendarSummary(entry.summary)
    );
    if (match) {
      calendar = match;
      reused = true;
...
export async function ensureManagedCalendarAclForUser(
  ...
  for (const email of uniqueEmails) {
    if (existingAcl && existingAcl.get(email) === role) {
      skipped += 1;
      continue;
    }

    try {
      const aclInsertUrl = `${aclBaseUrl}?sendNotifications=false`;
      await googleJsonRequest(accessToken, aclInsertUrl, {
        method: 'POST',
        body: JSON.stringify({
          role,
          scope: {
      ...
      if (error instanceof GoogleSyncException && error.status === 409) {
        skipped += 1;
        continue;
      }
      errors += 1;
